...
import radiolink_teleop.joystick_ros as j
...
if __name__ == '__main__':
    ...
    rospy.Subscriber("disable", Bool, disable_callback, queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        status = joy.get_joy_status()#j.CONNECTED#
        # print(status)
        if status == j.CONNECTED:
            ...
            if t8s[j.R2] == 0 and is_standup == 1:
                mode = walk_mode
            ...
            # if R2 in down position
            elif t8s[j.R2] == 1 and is_standup == 1:
                mode = walk_mode
                ...
                if abs(cmd_vel_msg.linear.x) < 0.01:
                    cmd_vel_msg.linear.x = 0 
                if cmd_vel_msg.linear.x < -max_speed_x/2:
                    cmd_vel_msg.linear.x = -max_speed_x/2 
                if abs(cmd_vel_msg.linear.y) < 0.01:
                    cmd_vel_msg.linear.y = 0
                if abs(cmd_vel_msg.angular.z) < 0.01:
                    cmd_vel_msg.angular.z = 0
                else:
                    cmd_vel_msg.linear.y = 0
            ...
            # if R2 in up position
                ...
                cmd_pose_msg.angular.x = t8s[j.LX] * max_body_wx * (1 - np.abs(t8s[j.LY])) * (1 - np.abs(t8s[j.RY]))
                cmd_pose_msg.angular.y = t8s[j.RY] * max_body_wy * (1 - np.abs(t8s[j.LY]))
                cmd_pose_msg.angular.z = t8s[j.RX] * max_body_wz * (1 - np.abs(t8s[j.LY]))
                ...
                if abs(cmd_pose_msg.angular.z) < 0.02:
                    cmd_pose_msg.angular.z = 0
            ...
            if t8s[j.L2] == 1 and is_standup == 1:
                action = GIVE_HAND # give hand
            elif t8s[j.L2] == -1 and is_standup == 1:
                action = SIT_DOWN # sit down

            if t8s[j.L1] > 0.1 and is_standup == 1:
                action = SIDE_ROLL

            # print(t8s[j.L2])
            # print(f"{action} {action_prev} {is_standup}")
        ...
        status_pub.publish(not(bool(joy_fault)))
        rate.sleep()
